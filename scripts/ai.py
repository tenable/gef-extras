"""
This command sends information on the current debugging context to a Large Language
Model operated by OpenAI or Anthropic, and asks it a question supplied by the user.
It then displays the model's response to that question to the user.
"""
...
"""
    ...
    def do_invoke(self, argv: List[str]):
        ...
        parser.add_argument("question", type=str, default=None,
                            nargs="+", help="The question to ask.")
        parser.add_argument(
            "-M", "--model", default="gpt-3.5-turbo", help="The OpenAI model to use.")
        parser.add_argument("-t", "--temperature", default=0.5,
                            type=float, help="The temperature to use.")
        parser.add_argument("-m", "--max-tokens", default=100,
                            type=int, help="The maximum number of tokens to generate.")
        parser.add_argument("-v", "--verbose", action="store_true",
                            help="Print the prompt and response.")
        ...
        try:
            ...
            vendor = "Anthropic" if args.model.startswith(
                "claude") else "OpenAI"
            gef_print(
                f"To query the {args.model} model, you must set the {vendor.upper()}_API_KEY environment variable to the {vendor} API key.")
            return
        ...
        answer = query(prompt, model=args.model, max_tokens=args.max_tokens,
                       temperature=args.temperature, api_key=api_key).strip()
